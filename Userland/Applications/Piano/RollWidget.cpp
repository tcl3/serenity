...
void RollWidget::mousemove_event(GUI::MouseEvent& event)
{
    ...
    if (m_mousedown_event.value().button() == GUI::MouseButton::Primary) {
        ...
        u32 const on_sample = AK::round(roll_length * (static_cast<double>(min(x_start, x_end)) / m_num_notes));
        u32 const off_sample = AK::round(roll_length * (static_cast<double>(max(x_start, x_end) + 1) / m_num_notes)) - 1;
