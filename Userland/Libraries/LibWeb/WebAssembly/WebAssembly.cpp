...
bool validate(JS::VM& vm, JS::Handle<WebIDL::BufferSource>& bytes)
{
    ...
    auto context = current_web_assembly_context(*vm.current_realm());
    if (context.abstract_machine().validate(compiled_module->module).is_error())
        return false;
...
WebIDL::ExceptionOr<JS::Value> compile(JS::VM& vm, JS::Handle<WebIDL::BufferSource>& bytes)
{
    ...
    if (compiled_module_or_error.is_error()) {
        ...
        auto x = compiled_module_or_error.release_value();
        auto module_object = vm.heap().allocate<Module>(realm, realm, move(x));
...
WebAssemblyContext current_web_assembly_context(JS::Realm&)
{
    VERIFY_NOT_REACHED();
}
...
JS::ThrowCompletionOr<NonnullOwnPtr<Wasm::ModuleInstance>> instantiate_module(JS::VM& vm, Wasm::Module const& module)
{
        ...
        for (Wasm::Linker::Name const& import_name : linker.unresolved_imports()) {
            ...
            TRY(import_name.type.visit(
                [&](Wasm::TypeIndex index) -> JS::ThrowCompletionOr<void> {
                    ...
                    auto address = context.abstract_machine().store().allocate(move(host_function));
                ...
                [&](Wasm::GlobalType const& type) -> JS::ThrowCompletionOr<void> {
                    ...
                    if (import_.is_number() || import_.is_bigint()) {
                        ...
                        address = context.abstract_machine().store().allocate({ type.type(), false }, cast_value);
                    } else {
    ...
    auto instance_result = context.abstract_machine().instantiate(module, link_result.release_value());
...
JS::ThrowCompletionOr<NonnullRefPtr<CompiledWebAssemblyModule>> parse_module(JS::VM& vm, JS::Object* buffer_object)
{
    ...
    auto context = current_web_assembly_context(*vm.current_realm());
    if (auto validation_result = context.abstract_machine().validate(module_result.value()); validation_result.is_error()) {
        // FIXME: Throw CompileError instead.
    ...
    context.add_module(compiled_module);
...
JS::NativeFunction* create_native_function(JS::VM& vm, Wasm::FunctionAddress address, ByteString const& name)
{
    ...
    auto context = current_web_assembly_context(realm);
    context.abstract_machine().store().get(address)->visit([&](auto const& value) { type = value.type(); });
    if (auto entry = context.get_function_instance(address); entry.has_value())
        return *entry;
    ...
    auto function = JS::NativeFunction::create(
        ...
        [address, type = type.release_value()](JS::VM& vm) -> JS::ThrowCompletionOr<JS::Value> {
            ...
            auto context = current_web_assembly_context(realm);
            auto result = context.abstract_machine().invoke(address, move(values));
    ...
    context.add_function_instance(address, function);
    return function;
}
...
JS::ThrowCompletionOr<Wasm::Value> to_webassembly_value(JS::VM& vm, JS::Value value, Wasm::ValueType const& type)
{
    ...
    case Wasm::ValueType::NullFunctionReference: {
        ...
        if (value.is_function()) {
            ...
            auto context = current_web_assembly_context(*vm.current_realm());
            for (auto& entry : context.function_instances()) {
